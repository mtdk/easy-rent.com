...
#
# 用法:
#   ./scripts/show-nightly-maintenance-history.sh [limit] [--status=success|failed|all] [--mode=preview|commit|all] [--from=<datetime>] [--to=<datetime>] [--trend-window=<n>] [--fail-rate-threshold=<0~1>] [--emit-risk-alert] [--risk-alert-dir=<path>] [--risk-webhook-url=<url>] [--summary] [--csv=<path>] [--json] [--json-file=<path>]
...
FILES=( $(ls -1t "$LOG_DIR"/nightly-maintenance_*.json 2>/dev/null | head -n "$LIMIT" || true) )
...
json_value() {
  ...
  local key="$2"
  local value
  value="$(grep -m1 "\"$key\"" "$file" | sed -E "s/^[[:space:]]*\"$key\"[[:space:]]*:[[:space:]]*//; s/,[[:space:]]*$//; s/^\"//; s/\"$//")"
  printf '%s' "$value"
}
...
pass_status() {
  local status="$1"
  [[ "$STATUS_FILTER" == "all" || "$STATUS_FILTER" == "$status" ]]
}

pass_mode() {
  local mode="$1"
  [[ "$MODE_FILTER" == "all" || "$MODE_FILTER" == "$mode" ]]
}
...
append_json_record() {
  ...
  local item
  item="{\"timestamp\":\"$(json_escape "$ts")\",\"mode\":\"$(json_escape "$mode")\",\"status\":\"$(json_escape "$status")\",\"duration_seconds\":\"$(json_escape "$duration")\",\"exit_code\":\"$(json_escape "$exit_code")\",\"report\":\"$(json_escape "$report")\"}"
...
emit_risk_alert() {
  ...
  notification_message_id="${RISK_NOTIFICATION_MESSAGE_ID_PREFIX}-nightly-maintenance-history-${risk_level}-${now_compact}"
  risk_notification_message_id="$notification_message_id"

  mkdir -p "$RISK_ALERT_DIR"
  risk_alert_file="$RISK_ALERT_DIR/nightly-maintenance-history-risk-alert_${risk_level}_${now_compact}.json"

  cat > "$risk_alert_file" <<EOF
{
  "task": "nightly-maintenance-history",
...
if [[ -n "$CSV_OUTPUT" ]]; then
  echo "timestamp,mode,status,duration_seconds,exit_code,report" > "$CSV_OUTPUT"
fi
...
printf "%-22s %-8s %-8s %-10s %-8s %-32s\n" "timestamp" "mode" "status" "duration" "exit" "report"
printf "%-22s %-8s %-8s %-10s %-8s %-32s\n" "----------------------" "--------" "--------" "----------" "--------" "--------------------------------"
...
for file in "${FILES[@]}"; do
  ...
  duration="$(json_value "$file" "duration_seconds")"
  ...
  base_name="${file##*/}"
  printf "%-22s %-8s %-8s %-10s %-8s %-32s\n" "$ts" "$mode" "$status" "$duration" "$exit_code" "$base_name"

  if [[ -n "$CSV_OUTPUT" ]]; then
    printf '"%s","%s","%s","%s","%s","%s"\n' "$ts" "$mode" "$status" "$duration" "$exit_code" "$base_name" >> "$CSV_OUTPUT"
  fi

  append_json_record "$ts" "$mode" "$status" "$duration" "$exit_code" "$base_name"
