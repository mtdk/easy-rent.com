...
#
# 用法:
#   ./scripts/cleanup-restore-history.sh [mode] [keep_days] [keep_count]
#
# 参数:
...
while IFS= read -r f; do
  reports+=("$f")
done < <(ls -1t "$LOG_DIR"/restore-verify_*.json 2>/dev/null || true)
...
done < <(ls -1 "$LOG_DIR"/restore-verify_*.log 2>/dev/null || true)
