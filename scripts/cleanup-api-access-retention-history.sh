...
# 清理 API 审计日志保留历史日志与报告
#
# 用法:
#   ./scripts/cleanup-api-access-retention-history.sh [mode] [keep_days] [keep_count]
#
# 参数:
...
while IFS= read -r f; do
  reports+=("$f")
done < <(ls -1t "$LOG_DIR"/api-access-retention_*.json 2>/dev/null || true)

if [[ "${#reports[@]}" -eq 0 ]]; then
  echo "未发现可清理 API 审计保留历史报告文件"
  exit 0
fi
...
done < <(ls -1 "$LOG_DIR"/api-access-retention_*.log 2>/dev/null || true)
